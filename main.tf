...
module "pool" {
  ...
  instance_enabled = local.worker_pool_enabled

  instance_tenancy_ocid        = local.tenancy_ocid
  instance_region              = local.region
  instance_swarm_worker_count  = local.worker_pool_node_count
  instance_swarm_master_ip     = module.master.instance_private_ip
  instance_label_prefix        = local.label_prefix
  instance_label_postfix       = local.label_postfix
  instance_compartment_id      = local.worker_pool_compartment_id != "" ? local.worker_pool_compartment_id : local.compartment_id
  instance_availability_domain = 1
  instance_vcn_id              = (local.worker_pool_vcn_id != "") ? local.worker_pool_vcn_id : local.vcn_id
  instance_subnet_id           = (local.worker_pool_subnet_id != "") ? local.worker_pool_subnet_id : local.subnet_id
  instance_nat_route_id        = ""
  instance_nsg_ids             = []
  instance_image_id            = (local.worker_pool_image_id != "") ? local.worker_pool_image_id : local.image_id
  instance_shape               = (local.worker_pool_shape != {}) ? local.worker_pool_shape : local.shape
  instance_upgrade             = true
  instance_ssh_public_key      = local.ssh_public_key
  instance_ssh_public_key_path = local.ssh_public_key_path
  instance_timezone            = local.timezone
